...
namespace Vivencia19ManhaAPI.Business
{
    public class CursoBusiness
    {
        ...
        public void InserirCurso(Models.TbCurso curso)
        {
            ...
            if (validar == true)
                throw new ArgumentException("Ocorreu um erro. Impossivel cadastrar duas vezes o mesmo curso no sistema"); 
            
            if(curso.NrCapacidadeMaxima <= 0 || curso.NrCapacidadeMaxima > 50)
                throw new ArgumentException("Ocorreu um erro. A capacidade de alunos deve ser maior que zero e igual ou menor a cinquenta");

            if(curso.DsCategoria == string.Empty)
                throw new ArgumentException("Ocorreu um erro. Insira um categoria para o curso");
